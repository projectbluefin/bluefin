...
bluefin-lts (PRs→main; main→lts on release)
...
bluefin-lts → main  (migration to testing-first tracked in bluefin-lts/346; use main until that lands)
...
| `bluefin-lts` | `main` | `testing→main` | `execute-release.yml` copies `:testing`→`:lts` |
| `dakota` | `testing` | `testing→main` | `execute-release.yml` fires on push to main |

Never target `main` directly for feature work in `bluefin` or `dakota`. `bluefin-lts` stays on `main` until bluefin-lts/346 lands.
