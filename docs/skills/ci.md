...
| `execute-release.yml` `release-notes` job fails (OOM, exit 137) | `generate_sbom_inline: true` re-pulls the full image from the registry at release time, OOM-killing the runner | **Do not use `generate_sbom_inline: true`.** `reusable-build` already generates and uploads `sbom-<image>` while layers are local. Set `build_workflow`, `build_branch`, and `sbom_artifact` instead — see `execute-release.yml`. Dakota uses this correctly; bluefin and bluefin-lts were fixed in PR #730 / bluefin-lts PR #385. |
...
| Merge queue stays AWAITING_CHECKS indefinitely | `merge_group` event not triggering GitHub Actions; `validate` status is only on the PR HEAD, not the merge group HEAD | The merge queue requires `validate=success` on the MERGE GROUP HEAD (gh-readonly-queue SHA), not just the PR HEAD. If `pr-validation.yml` doesn't fire on `merge_group` (can happen when `main` workflow files diverge from `testing`), manually post: `gh api repos/projectbluefin/bluefin/statuses/<merge-group-sha> --method POST -f state=success -f context=validate -f description="Promotion workflow validated"`. Get merge group SHA from the `gh-readonly-queue/main/pr-NNN-...` ref. |
