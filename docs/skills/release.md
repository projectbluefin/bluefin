...
- LTS stream specifics → [lts.md](lts.md)
...
- Pulls the promoted image via `skopeo copy` → `oci-dir`
- Scans with Syft (spdx-json format) to produce a full package inventory
...
**Use `generate_sbom_inline: true` in `execute-release.yml`.** The artifact-based approach (`generate_sbom_inline: false` + `build_workflow`/`build_branch`/`sbom_artifact`) requires:
1. A recent successful build that uploaded `sbom-$IMAGE_NAME` artifact
2. The `gen-sbom` Justfile target producing SPDX JSON (currently outputs Syft JSON — format mismatch with `create-release` validation)

Until both are fixed, the artifact-based approach fails with "Artifact not found" or SBOM validation errors.
