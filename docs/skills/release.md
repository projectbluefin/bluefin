...
**Use `generate_sbom_inline: true` in `execute-release.yml`.** Syft scans `registry:IMAGE:stable` directly using only the RPM database cataloger. All notable packages (kernel, GNOME, Podman, systemd, Mesa, PipeWire, Flatpak, bootc) are RPMs. `GOMEMLIMIT=6GiB GOGC=25` cap the Go heap to prevent OOM on 7 GB runners.
