...
```bash
# 1. ALWAYS delete the existing GitHub Release first (keeps the git tag)
gh release delete <tag> --repo projectbluefin/bluefin --yes   # NOT --cleanup-tag
...
**Why you must delete first — `create-release` execution order:**

The `bootc-build/create-release` composite action runs expensive operations in this order:
1. Validate SBOM
2. Fetch previous release SBOM
3. Diff SBOMs (massive if fallback 0-package SBOM — shows ~2000 "removed" packages)
4. Render release card PNG (~2000 chips → hangs or OOMs)
5. Generate changelog
6. **Only then** check if the GitHub Release already exists (idempotency check)

If you re-trigger without deleting the existing release, steps 3–4 still run with the fallback SBOM. The release card renderer attempts to draw ~2000 "removed" package chips and hangs for 40+ minutes before reaching the idempotency check in step 6. Force-cancel and delete the release first.

**Fallback SBOM causes hang:** When Syft times out or fails (8-min timeout, `continue-on-error: true`), a minimal 0-package fallback SBOM is written. The SBOM diff against the previous real SBOM (~2000 packages) produces a change list showing all packages removed. Release card rendering with 2000 chips hangs or OOMs.
