...
The cached initramfs built by `build_files/base/04-install-kernel-akmods.py`
and the fallback generated by `build_files/base/19-initramfs.sh` must use the
same compression and module options. Keep XZ compression explicit in both
paths: the smaller on-disk image reduces the contiguous memory GRUB needs when
loading module-heavy variants such as `bluefin-nvidia`.
