...
| `push-image` fails: `permission denied` on `/run/user/1001/containers/auth.json` | `sudo buildah push` inherits `XDG_RUNTIME_DIR=/run/user/1001` from the calling shell. Ubuntu 24.04 runners (AppArmor + systemd-logind) deny root processes access to that path even via sudo. | In `bootc-build/push-image/action.yml`: pass `--authfile "${RUNNER_TEMP}/push-auth.json"` to both `sudo podman login` and `sudo buildah push`. `RUNNER_TEMP` is always writable by all processes regardless of AppArmor. |
| `execute-release.yml` `startup_failure` — workflow file issue | Two causes: (1) `>-` block scalar YAML syntax for `cosign_identity_regexp` in the `execute` job triggers a GitHub Actions parser bug. (2) `timeout-minutes:` is not valid on `uses:` jobs (reusable workflow calls). | Use inline scalar (no `>-`) for `cosign_identity_regexp`. Remove `timeout-minutes:` from any job that uses `uses:`. actionlint catches both. |
| `execute-release.yml` never fires automatically | `pull_request: closed` trigger is unreliable for reusable callers. | Use `push: branches: [main]` + `workflow_dispatch` with a `check-trigger` job that greps the squash commit message for `^ci(promote):`. Mirror the lts pattern. |
| `post-testing-e2e` always fails with "workflow file issue" | `timeout-minutes:` on a job that calls a reusable workflow via `uses:` is invalid. GitHub's parser rejects the file at startup before any jobs run. | Remove `timeout-minutes:` from the `run-e2e` job. Only `name`, `uses`, `with`, `secrets`, `needs`, `if`, and `permissions` are allowed. actionlint: `syntax-check`. |
| `reusable-release.yml` inline SBOM generation killed after ~4 min (SIGTERM / exit 143) | Syft scanning a full GNOME desktop image via the default scheme downloads and decompresses the entire image, exhausting runner RAM. | In `reusable-release.yml`: use `registry:` prefix so Syft streams layers without full decompression, add `--select-catalogers "rpm"` to limit scope, wrap with `timeout 180 ... \|\| write-stub`. All notable packages are RPM packages. |
| `dracut EXDEV` in bluefin-lts push builds | `%posttrans` kernel-install hook calls dracut via `kernel-install add`, which internally does `rename(tmpfs → overlay)` — always EXDEV regardless of `tmpdir` setting in `dracut.conf.d`. | Install kernel RPMs with `--setopt=tsflags=noscripts` to skip the `%posttrans` hook, then run `dracut --force --tmpdir /boot` explicitly. Mirrors the pattern in `20-nvidia.sh`. |
